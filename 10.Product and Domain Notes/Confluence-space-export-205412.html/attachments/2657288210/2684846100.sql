-- Tes1>> Get the #BloombergPrice table, i found out most of the Bloomberg price missing the PX_AS/BID/LAST price, only the H.SecurityPrice have prices
...
Select *
FROM #BloombergPrice
WHERE [Exchange Security] IN ('AIR', 'GMT', 'NZX', 'HFL', 'TWH', 'USM', 'AUE', 'GPR') -- this is to filter to the spefic list of secirity that isers have questions





------Test2:  this is to check the Fustion database price value

DECLARE @AsAtDate2 DATE = '2024-07-31';

-- Drop the temporary table if it already exists
IF OBJECT_ID('tempdb..#FusionPrice') IS NOT NULL 
    DROP TABLE #FusionPrice;
...
WHERE a.status = 1
AND ph.priceDate = @AsAtDate2;


Select *
FROM #FusionPrice
WHERE [AssetCode] IN ('AIR', 'GMT', 'NZX', 'HFL', 'TWH', 'USM', 'AUE', 'GPR')





------Test 3: this scritps is to cross comared Bloom 1 Web page data with our SQL CIP bloomberge data
